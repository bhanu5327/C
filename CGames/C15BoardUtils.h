...
Board * createBoard(int rows, int cols){
	if (rows <= 0 || cols <= 0) return NULL;
		Board *changeThis = (Board*)malloc(sizeof(Board)); int i;
int **table1 = (int**)malloc(rows*sizeof(int*));
for ( i = 0; i < 4; i++)
{
	table1[i] = (int*)malloc(cols* sizeof(int));
}
/*for (i = 0; i < 4;i++)
{
	for (int j = 0; j < 4; j++)
...
void initializeBoard(Board * source, int *tobeCopiedArray, int rows, int cols)
{
	...
	else
	{
		for (int i = 0; i < rows; i++)
		{
			...
			{
				source->table[i][j] = tobeCopiedArray[h++];
			}
		}
