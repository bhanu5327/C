...
struct ObjectNode * createIntObjectNode(int *number, int(*equals)(void *, void *)) {
	...
	struct ObjectNode *k = (struct ObjectNode *)malloc(sizeof(struct ObjectNode) );
	k->value = (void*)number;
	k->equals = equals;
	return k;
}
...
struct ObjectNode ** combine_common_elements(struct ObjectNode ** objectNodes1, struct ObjectNode ** objectNodes2, int len1, int len2, int *commonElementsLength) {
	...
	int i = 0, j = 0, h = 0; int m; int n;
	int p = 0; int* b = b = (int*)malloc(len1*sizeof(int));
	struct ObjectNode ** result;
	*commonElementsLength = 0;
	for (i = 0; i < len1; i++)
	{
		h = 0;
		for (int k = 0; k < i; k++)
		{
			m = *(int *)objectNodes1[i]->value; n = *(int*)objectNodes1[k]->value;
			if (m == n)
			{
				h = 1;
				break;
		...
		if (h == 0)
		{
			for (j = 0; j < len2; j++)
			{
				int k = equalsInt1(objectNodes1[i]->value, objectNodes2[j]->value);
				if (k == 1)
				{
					*commonElementsLength = *commonElementsLength + 1;
					//result[p++] = (struct ObjectNode*)objectNodes1[i]->value;
					b[p++] = i;
					break;
				}
			}
	...
	result= (struct ObjectNode **)malloc(sizeof(struct ObjectNode *) *(*commonElementsLength));
	for (int k = 0; k < p; k++)
	{
		result[k] =(objectNodes1[b[k]]);
	}
	//result=(struct objectNode*)realloc(result, *commonElementsLength);
