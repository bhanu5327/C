...
void task5() {
	int a = 5;
	int b=typeCastVoidToInt((void *)&a);
	printf(" value is %d", b);
}
...
void typeCastToAllDataTypes(void * long_ptr, void * float_ptr, void * char_ptr, void * str_ptr) {
	...
	printf("Value stored at long_ptr is %c\n", *(char *)char_ptr);
	printf("Value stored at long_ptr is %s\n", (char *)str_ptr);
	//Other code
}
...
void task7() {
	...
	x = (float*)getFloatAsVoidStar();
	y=(int *)getIntAsVoidStar();
	z = (char*)getStringAsVoidStar();
	printf("Float is %f", *x);
	printf("Int is %d", *y);
	printf("String is %s", z);
}
...
//Optional Do for 111111111111111111111111111111111compareStrings also.
...
void ** getTwoGreater(void *addr1, void *addr2, void *addr3, int(*compare)(void *, void *)) {
	...
	void ** result = (void **)malloc(sizeof(void *) * 2); int p, q, r;
	p = compare(addr1, addr2);

	q = compare(addr2, addr3);
	r = compare(addr1, addr3);
	if (p == 1 && r == 1)
	{
		result[0] = addr1;
		if (q == 1)
...
void task9() {
	...
	float x, y, z;
	r = (float **)getTwoGreater(&x, &y, &z, &compareFloat);
