...
{
	struct passKeyNode*temp; struct passKeyNode*head1; struct passKeyNode* sam; struct passKeyNode* sam1;
	struct passKeyNode*temp1; struct passKeyNode*last;
	temp = passKeyHead; int i = 1,j=0,k=1,min=-1,n,t;
		...
		else
		{
			...
			while (temp != NULL)
			{
				if (n < temp->num)
				{
					if (min == -1)
					{
						min = temp->num; sam1 = temp;
					}
					else if (min > temp->num)
					{
						min = temp->num; sam1 = temp;
					}
				}
			...
			t = sam->num; sam->num = sam1->num; sam1->num = t;
			temp = head1;
			while (temp != NULL)
			{
				temp1 = temp->next;
				while (temp1 != NULL)
				{
					...
					{
						t = temp->num; temp->num = temp1->num; temp1->num = t;
					}
					temp1 = temp1->next;
				}
				temp = temp->next;
