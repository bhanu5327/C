...
int * oddeven_sll(struct oddevennode *head){
	struct oddevennode* temp; struct oddevennode* odd; struct oddevennode* even;
	temp = head; int i = 0, j = 0, k; int *a=(int*)malloc(2*sizeof(int));
	...
	while (temp != NULL)
	{
		...
		if (temp->data % 2 != 0)
		{
			...
			{
				odd->random = temp; i++; odd = odd->random;
			}
		}
		else
		{
			...
			{
				even->random = temp; j++; even = even->random;
			}
		}
		temp =temp->next;
	}
