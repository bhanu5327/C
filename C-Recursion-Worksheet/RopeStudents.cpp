...
void height(struct student *temp, int k, int *min, int *max)
{
	...
	{
		k++;
		height(temp->first, k, min, max);
		//	if (temp->second!=NULL)
		height(temp->second, k, min, max);
		//if (temp->third!=NULL)
		height(temp->third, k, min, max);
	}
...
void findBallDistance(struct student *leaderStudent, int *minBallDistance, int *maxBallDistance)
{
	...
	struct student * temp = leaderStudent; int k = 0;
	*minBallDistance = 0; *maxBallDistance = 0;
	height(temp, k, minBallDistance, maxBallDistance);
}
